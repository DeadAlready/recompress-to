...
const xzBin = config.xz || '/usr/local/bin/xz';

function create(file) {
    debug('create xz', file);
    execFileSync(xzBin, ['-zk9', file]);
...
function toXz(file) {
    ...
    if(file.endsWith('.rar')) {
        ...
        dir = Gzip.unpack(file);
        if(!dir) {
            console.log('unprocessable file - nodir', file);
            return 0;
        }
        tar = file.substr(0, file.length - 4) + '.tar';
    }
